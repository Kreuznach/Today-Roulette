...
[1차 결과 화면 /result]
     ...
     [원래 결과 유지]  startAd()  ← watchAd() 내부: load → show
                  ...
                  │ false       │ true (보상 획득)
                  ↓            ↓
            [에러 메시지]  navigate('/roulette', { state: { isRetry: true } })
                                │
                                ↓
                    [룰렛 화면 /roulette (retry 모드)]
                    "한 번 더 돌려보세요" 헤딩 표시
                                │
                         룰렛 돌리기 버튼 클릭
                                │
                         spinRetry() + saveRetryResult()
                                │
                                ↓
                        [최종 결과 /final]
                        (usedAdRetry=true 뱃지)
...
```typescript
const { adState, errorMessage, startAd, reset } = useRewardAd()
// adState: 'idle' | 'loading' | 'done' | 'error'
```
...
spinRetry()    // () => RouletteFinalResult | null — 재추첨 실행 (RoulettePage retry 모드에서 호출)
saveRetryResult(result) // 재추첨 결과 저장 (RoulettePage retry 모드에서 호출)
```
...
| 환경 | `watchAd()` |
|------|------------|
| DEV (비 AIT 환경) | 1.5초 지연 후 `true` 반환 (Mock) |
| Production (AIT) | `GoogleAdMob` load → show 순차 실행 |
